...
#?(:cljs (def cursor r/cursor)
   :clj
   (defn cursor [!atom path]
     (reify clojure.lang.IAtom
       (swap [_this f] (get-in (swap! !atom update-in path f) path))
       clojure.lang.IDeref
       (deref [_this] (get-in (deref !atom) path)))))
...
(defonce !eval-counter (cursor !state [:eval-counter]))
(defonce !doc (cursor !state [:doc]))
(defonce !viewers (cursor !state [:viewers]))
...
(defn render-notebook [{xs :blocks :keys [package doc-css-class sidenotes? toc toc-visibility header footer]}
  ...
  (let #?(:clj []
          :cljs [_(hooks/use-effect #(swap! !expanded-at merge (navbar/->toc-expanded-at toc toc-visibility)) [toc toc-visibility])
                 !mobile-toc? (hooks/use-state (navbar/mobile?))
    ...
    [:div.flex
     #?(:cljs {:ref root-ref-fn})
     [:div.fixed.top-2.left-2.md:left-auto.md:right-2.z-10
      [dark-mode-toggle]]
     #?(:cljs (when (and toc toc-visibility)
      ...
      (into
       [:> #?(:cljs (.-div motion))
        (merge
         ...
         #?(:cljs (when (and toc (not (navbar/mobile?)))
...
(defn error-view [error]
  (let [!stack-expanded #?(:clj (atom false)
                           :cljs (hooks/use-state false))]
    [:div.bg-red-100.dark:bg-gray-800.px-6.py-4.rounded-md.text-xs.dark:border-2.dark:border-red-300.not-prose
     ...
     (when-let [stack (try
                        (->> (or (:stack error) (.-stack error))
                             ...
                             (drop 1)
                             (mapv str/trim))
                        (catch #?(:cljs js/Error :clj Exception) _ nil))]
...
;; TODO: drop this
#?(:cljs (defn read-string [s]
           (js/nextjournal.clerk.sci_env.read-string s)))


#?(:cljs (defn fetch! [{:keys [blob-id]} opts]
...
(defn render-elision [{:as fetch-opts :keys [total offset unbounded?]} _]
  #?(:clj [:div "TODO render-elision"]
     :cljs [view-context/consume :fetch-fn
            (fn [fetch-fn]
...
(defn render-number [num]
  [:span.cmt-number.inspected-value
   (if (#?(:cljs js/Number.isNaN :clj NaN?) num) "NaN" (str num))])
...
(defn throwable-view [{:as _error :keys [via trace]} opts]
  (let [!stack-expanded? #?(:cljs (nextjournal.clerk.render.hooks/use-state false) :clj (atom false))
        !caused-by-expanded? #?(:cljs (nextjournal.clerk.render.hooks/use-state false) :clj (atom false))
        unhandled (first via)]
...
(defn render-throwable [ex opts]
  (if (or (:stack ex) #?(:cljs (instance? js/Error ex)))
    [error-view ex]
...
#?(:cljs (defn valid-react-element? [x] (react/isValidElement x)))

#?(:cljs (defn inspect-presented
           ([x]
...
(defn inspect [value]
  #?(:cljs (r/with-let [!state   (r/atom nil)
...
#?(:cljs (defn with-fetch-fn [{:nextjournal/keys [presented blob-id]} body-fn]
  ...
  [#?@(:cljs [:> ErrorBoundary {:hash @!doc}]
   ...
   [:div.fixed.w-full.z-20.top-0.left-0.w-full
    ...
    (when-let [status (:status @!doc)]
      [exec-status status])]
   #?(:cljs (if-let [{:as wrapped-value :nextjournal/keys [blob-id]} (get-in @!doc [:nextjournal/value :error])]
...
#?(:cljs (defn atom-changed [var-name _atom old-state new-state]
           (when *sync*
             (ws-send! {:type :sync!
                        :var-name var-name
                        :patch (editscript/get-edits (editscript/diff old-state new-state {:algo :quick}))}))))

#?(:cljs (defn intern-atom! [var-name state]
...
#?(:cljs (defonce ^:private !synced-atom-vars
           (atom #{})))

#?(:cljs (defn sci-ns-unmap! [ns-sym var-sym]
...
#?(:cljs (defn intern-atoms! [atom-var-name->state]
...
#?(:cljs (defn re-eval-render-fns [doc]
...
#?(:cljs (defn ^:export set-state! [{:as state :keys [doc effects]}]
           (run-effects! effects)
...
#?(:cljs (defn apply-patch [x patch]
           (eval-cljs-evals (editscript/patch x (editscript/edits->script patch)))))

#?(:cljs (defn patch-state! [{:keys [patch effects]}]
           (run-effects! effects)
           (if (remount? patch)
...
#?(:cljs (defn process-eval-reply! [{:keys [eval-id reply error]}]
...
#?(:cljs (defn ->URL [^js href]
           (js/URL. href)))

#?(:cljs (defn path-from-url-hash [url]
           (-> url ->URL .-hash (subs 2))))

#?(:cljs (defn ->doc-url [url]
...
#?(:cljs (defn history-push-state [{:as opts :keys [path fragment replace?]}]
...
#?(:cljs (defn handle-history-popstate [^js e]
...
#?(:cljs (defn handle-initial-load [^js _e]
           (history-push-state {:path (subs js/location.pathname 1) :replace? true})))

#?(:cljs(defn utf8-decode [bytes]
          (.decode (js/TextDecoder. "utf-8") bytes)))

#?(:cljs (defn delay-resolve [v] (new js/Promise (fn [res] (js/setTimeout #(res v) 100)))))

#?(:cljs (defn read-response+show-progress [{:as state :keys [reader buffer content-length]}]
...
#?(:cljs (defn fetch+set-state [edn-path]
...
#?(:cljs (defn load->fetch [{:keys [current-path]} _e]
...
#?(:cljs (defn popstate->fetch [^js e]
           (when-some [edn-path (when (.-state e) (.. e -state -edn_path))]
             (.preventDefault e)
             (fetch+set-state edn-path))))

#?(:cljs (defn setup-router! [{:as state :keys [render-router]}]
...
#?(:cljs (defn ^:export mount []
           (when (and react-root (not hydrate?))
             (.render react-root (r/as-element [root])))))

#?(:cljs (defn ^:dev/after-load ^:after-load re-render []
           (swap! !doc re-eval-render-fns)
           (mount)))

#?(:cljs (defn ^:export init [{:as state :keys [render-router path->doc]}]
           (setup-router! state)
...
#?(:cljs (def consume-view-context view-context/consume))
