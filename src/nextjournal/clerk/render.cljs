...
(defn intern-atoms! [atom-var-name->state ignore-sync-existing-vars?]
  ;; sync-existing-vars? is false on replies from server generated by client's own swaps
  (let [vars-in-use (into #{} (keys atom-var-name->state))
    ...
    (doseq [[var-name value] atom-var-name->state]
      (if-let [existing-var (sci/resolve (sci.ctx-store/get-ctx) var-name)]
        (when-not ignore-sync-existing-vars? (reset! @existing-var value))
