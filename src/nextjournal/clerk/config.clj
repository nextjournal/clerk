...
(def viewer-lookup-hash (str/trim (slurp (io/resource "viewer-js-hash"))))
(def viewer-lookup-url (str gs-url-prefix "/lookup/" viewer-lookup-hash))
(def viewer-local-lookup (str (io/file cache-dir "lookup" viewer-lookup-hash)))
...
(def cached-viewer-lookup
  "Lookup of viewer js. Loaded from local file system after caching."
  (delay
    (edn/read-string
     (if (fs/exists? viewer-local-lookup)
       (slurp viewer-local-lookup)
       (let [lookup (slurp viewer-lookup-url)]
         (fs/create-dirs (fs/parent viewer-local-lookup))
         (spit viewer-local-lookup lookup)
         lookup)))))
